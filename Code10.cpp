...
// IFTTT setup
const char* iftttServer = "http://maker.ifttt.com/trigger/event_name/with/key/your_IFTTT_key";

WiFiServer server(80);

void sendDataToCloud(float temperature, float humidity, float pressure, int gasLevel, float additionalTemp) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin(serverName);
    http.addHeader("Content-Type", "application/json");

    String jsonPayload = "{\"temperature\":" + String(temperature) + ", \"humidity\":" + String(humidity) + ", \"pressure\":" + String(pressure) + ", \"gasLevel\":" + String(gasLevel) + ", \"additionalTemp\":" + String(additionalTemp) + "}";
...
void sendNotification(float temperature) {
  if (WiFi.status() == WL_CONNECTED && temperature > 30.0) { // Example condition for notification
    HTTPClient http;
    http.begin(iftttServer);
    http.addHeader("Content-Type", "application/json");
...
void loop() {
  ...
  sendDataToCloud(temperature, humidity, pressure, gasLevel, additionalTemp);
