...
void updateSensors() {
  ...
  // Activate buzzer if motion or gas is detected
  if (val == HIGH || gasLevel > 300) {
  ...
  uploadDataToFirebase(temp, hum, lightLevel, pirState, gasLevel);
}
...
void uploadDataToFirebase(float temp, float hum, int lightLevel, int pirState, float gasLevel) {
