...
Room rooms[ROOM_COUNT] = {
  {2, 4, 34, 32, 33},  // Room 1
  {5, 18, 35, 25, 26}, // Room 2
  {19, 21, 36, 27, 22} // Room 3
};
...
void setup() {
  ...
  for (int i = 0; i < ROOM_COUNT; i++) {
    ...
    pinMode(rooms[i].audioControlPin, OUTPUT);
  }
...
void executeCommand(int command) {
  switch (command) {
    ...
    case 7:
      ...
      break;
    case 8:
      controlAudio(true, 0); // Turn on audio in Room 0
      sendToCloud("Audio On in Room 0");
      break;
    case 9:
      controlAudio(false, 0); // Turn off audio in Room 0
      sendToCloud("Audio Off in Room 0");
      break;
    default:
