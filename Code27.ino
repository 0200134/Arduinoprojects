...
float gasLevel = 0; // Variable to store gas sensor value
int waterLevel = 0; // Variable to store water level sensor value
...
void updateSensors() {
  ...
  gasLevel = analogRead(GAS_SENSOR_PIN); // Read gas sensor
  waterLevel = analogRead(WATER_SENSOR_PIN); // Read water level sensor
  ...
  // Activate buzzer if motion or gas is detected or water is detected
  if (val == HIGH || gasLevel > 300 || waterLevel < 600) {
  ...
  uploadDataToFirebase(temp, hum, lightLevel, pirState, gasLevel, waterLevel);
}
...
void handleRoot() {
  ...
  html += "<p><a href='/control?cmd=lock'>Lock Door</a></p>";
  html += "<p><a href='/control?cmd=unlock'>Unlock Door</a></p>";
  html += "</body></html>";
...
void uploadDataToFirebase(float temp, float hum, int lightLevel, int pirState, float gasLevel, int waterLevel) {
  ...
  if (Firebase.pushFloat(firebaseData, "/gasLevel", gasLevel)) {
    Serial.println("Gas level data uploaded successfully");
  } else {
    Serial.println("Failed to upload gas level data");
  }

  if (Firebase.pushInt(firebaseData, "/waterLevel", waterLevel)) {
    Serial.println("Water level data uploaded successfully");
  } else {
    Serial.println("Failed to upload water level data");
  }
}