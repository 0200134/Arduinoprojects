...
#include <GoogleCloudPubSub.h> // Hypothetical library for Google Cloud Pub/Sub
...
const char* google_pubsub_project_id = "YOUR_GOOGLE_PUBSUB_PROJECT_ID";
const char* google_pubsub_topic = "YOUR_GOOGLE_PUBSUB_TOPIC";
...
void setup() {
  ...
  GooglePubSub_Init(google_pubsub_project_id, google_pubsub_topic); // Hypothetical function
}
...
void log_to_google_pubsub(String data) {
  // Hypothetical function to log data to Google Cloud Pub/Sub
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin("https://pubsub.googleapis.com/v1/projects/" + String(google_pubsub_project_id) + "/topics/" + String(google_pubsub_topic) + ":publish");
    http.addHeader("Content-Type", "application/json");
...
void loop() {
  ...
  if (motion == HIGH) {
    ...
    if (activityRecognized) {
      ...
      log_to_azure(data);
      log_to_google_pubsub(data); // Log to Google Cloud Pub/Sub
  ...
  log_to_google_pubsub(weatherData); // Log to Google Cloud Pub/Sub
